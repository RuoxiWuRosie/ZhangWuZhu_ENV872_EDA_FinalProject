...
Detrended Mann Kendall test's result further supports that electricity price has been statistically significantly increasing through the past 8 years.

Lastly, we hope to predict future minimal temperature and energy load needed based on historical data. We will use Autoregressive Integrated Moving Average (ARIMA) package to do a prilimary simple forcasting.
```{r}
library(forecast)
#energy.prediction <- auto.arima(energyload.daily.ts,seasonal = TRUE)
summary(energy.prediction)
# Check residuals
checkresiduals(electricity.prediction)
# Forecast
min.temp.forecasted <- forecast(electricity.prediction, h = 12)
plot(min.temp.forecasted)
summary(min.temp.forecasted)
```
